...
if [ "$1" == "-h" ] ; then
	...
	for f in "${files[@]}"; do
		...
		for t in "${types[@]}"; do 
			...
			echo "$t"
			./node_modules/mocha/bin/mocha testApp.js --type "$t" --testsuite test_cases/"$f".tsv --no-timeouts --reporter mocha-junit-reporter --reporter-options mochaFile=testReport/"$f"-"$t"-$(date +%y%m%d%H%M).xml
		done
	done
fi
