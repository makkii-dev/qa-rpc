...
*/
		...
		it(`${testRow.prefix}:${testRow.testDescription}`, (done)=>{
			let helperfunc = testRow.helper? helper[testRow.helper]:(null,a,b,c,done)=>{return{RUNTIME_VARIABLES:a,testRow:b,VERIFY_VARIABLES:c,done:done};};
			let helperParams = testRow.helper_params? testRow.helper_params:null;
			let validPreFunc
			let validPostFunc
			helperfunc(helperParams,RUNTIME_VARIABLES,testRow,VERIFY_VARIABLES,done)
				.then(validPreFunc)
				.then(runOneRow)
				.then(validPostFunc)
				.catch((e)=>{done(e);});
